...
def main():
    ...
    for skill_dir in sorted(skills):
        ...
        needs_reorg = (
            audit["python_files_in_root"] or
            audit["md_files_in_root"] or
            audit["data_files_in_root"] or
            not audit["has_scripts_dir"] or
            not audit["has_references_dir"]
        )
        ...
        print(f"  scripts/: {'[Y]' if audit['has_scripts_dir'] else '[N]'} ({len(audit['script_files'])} files)")
        print(f"  references/: {'[Y]' if audit['has_references_dir'] else '[N]'} ({len(audit['reference_files'])} files)")
        ...
        if not needs_reorg:
            print(f"  [OK] Compliant with template")
