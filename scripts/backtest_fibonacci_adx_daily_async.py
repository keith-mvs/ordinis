#!/usr/bin/env python3
"""
GPU-Accelerated Fibonacci ADX Backtest - Daily Timeframe (Async Version)
Properly handles async strategy signals.
"""
...
async def backtest_daily_async(df: pd.DataFrame, params: dict) -> dict:
    """Run async backtest on daily data with proper position management."""
    ...
    min_required = strategy.get_required_bars()
    
    for i in range(max(lookback, min_required), len(df)):
        ...
        # Entry logic - pass ALL data up to current bar (not just a window)
        if position == 0:
            # Include all data up to and including current bar
            lookback_df = df.iloc[:i + 1].copy()
            
            try:
                # Properly await the async signal generation
                signal = await strategy.generate_signal(lookback_df, df.index[i])
                
                if signal is not None:
                    signals_generated += 1
                    
                    if signal.is_actionable(min_probability=0.5):
...
async def main():
    ...
    print("3-Year Historical Data | Async Signal Generation")
    print("=" * 80)
    print()
    ...
    for cat_name, symbols, cat_key in categories:
        ...
        for symbol in symbols:
            ...
            result = await backtest_daily_async(df, params)
            ...
            print(f"\n  {status} Return: {result['total_return']*100:.2f}% | "
                  ...
                  f"Signals: {result['signals_generated']} ({result['actionable_signals']} actionable) | "
    ...
    output_file = output_path / f"daily_backtest_async_{timestamp}.json"
