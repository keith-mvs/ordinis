...
def yield_to_worst(ytm: float, ytc: Optional[float] = None) -> float:
    """
...
def calculate_forward_rates(spot_rates: np.ndarray, 
    ...
    for i in range(1, n):
        ...
        forward_rates[i] = (
            ((1 + spot_rates[i]) ** maturities[i] /
             (1 + spot_rates[i-1]) ** maturities[i-1]) - 1
        )
    
    return forward_rates


def analyze_yield_curve(maturities: np.ndarray, 
...
def _interpret_curve(shape: str, slope: float) -> str:
    ...
    interpretations = {
        "Normal (upward sloping)": 
            "Economic expansion expected; higher long-term rates compensate for time risk",
        "Inverted (downward sloping)":
            "Recession signal; investors expect rates to fall as economy weakens",
        "Flat":
            "Economic uncertainty; transition period between expansion and contraction"
    }
