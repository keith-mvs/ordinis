...
class CoveredCallParameters:
    ...
    delta_target: Optional[float] = 0.30  # Target delta if using delta selection
...
class CoveredCallStrategy:
    ...
    def construct_position(self, 
        ...
        optimal_call = self._select_optimal_call(
            target_strike, options_chain, stock_price
        )
    ...
    def _select_optimal_call(self, target_strike: float, 
                            options_chain: Dict, stock_price: float) -> Optional[Dict]:
    ...
    def _option_meets_criteria(self, option: Dict, target_strike: float, 
        ...
        return (
            strike_diff < 0.025 and  # Within 2.5% of target
            premium_yield >= self.params.min_premium_yield and
            option['open_interest'] >= 50 and
            option['volume'] >= 10
        )
    
    def _score_option(self, option: Dict, stock_price: float, 
