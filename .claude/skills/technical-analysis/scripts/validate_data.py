...
def validate_data(data: pd.DataFrame) -> tuple[bool, list[str]]:
    ...
    if len(data) < min_length:
        errors.append(f"Warning: Only {len(data)} rows. " f"Recommended minimum: {min_length}")
    ...
    if len(outliers) > 0:
        errors.append(f"Warning: Found {len(outliers)} potential outliers " f"(>50% daily moves)")
