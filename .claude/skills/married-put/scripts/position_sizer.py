...
def portfolio_heat_check(
    positions: list, total_portfolio_value: float, max_portfolio_risk: float = 0.02
) -> dict:
    """
...
def diversification_calculator(
    capital_available: float, num_positions: int, position_overlap: float = 0.10
) -> dict:
    """
...
if __name__ == "__main__":
    ...
    pos2 = calculate_position_size(
        capital_available=25000, stock_price=52.75, put_premium=2.35, max_position_pct=0.40
    )
    ...
    pos3 = calculate_position_size(
        capital_available=50000, stock_price=175.50, put_premium=6.25, max_position_pct=0.30
    )
    ...
    print(
        f"Total Risk: ${heat['total_risk_dollars']:,.2f} "
        f"({heat['total_heat_pct']:.2f}% of portfolio)"
    )
    print(
        f"Max Acceptable Risk: ${heat['max_acceptable_risk']:,.2f} "
