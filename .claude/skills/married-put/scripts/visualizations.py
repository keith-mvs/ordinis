...
def plot_married_put_payoff(
    ...
    put_pl = np.where(
        prices <= put_strike, (put_strike - prices) * shares - position.put_cost, -position.put_cost
    )
    ...
    if show_components:
        ...
        ax.plot(prices, put_pl, "--", color=COLORS["put"], linewidth=2, label="Put Only", alpha=0.7)
    ...
    ax.axvline(
        ...
        linewidth=1.5,
        alpha=0.7,
        label=f"Initial Stock: ${stock_price:.2f}",
    )
    ax.axvline(
        x=put_strike,
        color=COLORS["put"],
        ...
        linewidth=1.5,
        alpha=0.7,
        label=f"Put Strike: ${put_strike:.2f}",
    )
    ax.axvline(
        x=position.breakeven_price,
        color=COLORS["breakeven"],
        linestyle="--",
        linewidth=2,
        label=f"Breakeven: ${position.breakeven_price:.2f}",
    )
...
def plot_strike_comparison(
    ...
    for strike, premium, label, color in zip(strikes, premiums, labels, colors, strict=False):
    ...
    for i, (prot, cost, label) in enumerate(zip(protections, costs, labels, strict=False)):
        ax2.annotate(
            label,
            (prot, cost),
...
def plot_expiration_comparison(
    stock_price: float,
    put_strike: float,
    premiums_dict: dict,
    shares: int = 100,
    ...
    for exp, d in zip(expirations, days, strict=False):
    ...
    # Add value labels
    for bar, cost in zip(bars2, monthly_costs, strict=False):
        height = bar.get_height()
        ax2.text(
            bar.get_x() + bar.get_width() / 2.0,
            height,
