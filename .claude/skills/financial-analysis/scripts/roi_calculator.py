...
class ROICalculator:
    ...
    def _create_assumptions_sheet(self):
        ...
        # Add assumptions
        assumptions = [
            ('Initial Investment', self.assumptions.initial_investment, 'USD', 'Capital budget', 'Upfront cost'),
            ...
            ('Salvage Value', self.assumptions.salvage_value, 'USD', 'Estimate', 'End-of-project value'),
            ('Project Life', len(self.assumptions.annual_cash_flows), 'Years', 'Project plan', 'Analysis period'),
        ]
        ...
        for row, assumption in enumerate(assumptions, 4):
            for col, value in enumerate(assumption, 1):
                ...
                if col == 2:  # Value column
                    cell.fill = PatternFill(start_color=COLORS['input'], 
                                           end_color=COLORS['input'], 
                                           fill_type='solid')
    ...
    def _create_cash_flow_analysis(self):
        ...
        for row, item in enumerate(line_items, 4):
            ws[f'A{row}'] = item
            ws[f'A{row}'].font = Font(bold=True if item in ['Total Cash Flow', 'Cumulative PV'] else False)
        ...
        for col in range(2, 3 + len(self.assumptions.annual_cash_flows)):
            ...
            ws[f'{col_letter}7'].fill = PatternFill(start_color=COLORS['calculation'], 
        ...
        for col in range(3, 3 + len(self.assumptions.annual_cash_flows)):
            ...
            ws[f'{col_letter}11'].fill = PatternFill(start_color=COLORS['output'], 
                                                      end_color=COLORS['output'], 
                                                      fill_type='solid')
    ...
    def _create_metrics_sheet(self):
        ...
        # Key metrics
        metrics = [
            ('Net Present Value (NPV)', f'=\'Cash Flows\'!{get_column_letter(2 + len(self.assumptions.annual_cash_flows))}11', '$#,##0'),
            ('Internal Rate of Return (IRR)', f'=IRR(\'Cash Flows\'!B7:{get_column_letter(2 + len(self.assumptions.annual_cash_flows))}7)', '0.00%'),
            ('Profitability Index', f'=(B3+InitialInvestment)/InitialInvestment', '0.00'),
            ('Payback Period (Years)', f'=MATCH(TRUE, \'Cash Flows\'!B8:{get_column_letter(2 + len(self.assumptions.annual_cash_flows))}8>=0, 0)-1', '0.0'),
            ('Discounted Payback (Years)', f'=MATCH(TRUE, \'Cash Flows\'!B11:{get_column_letter(2 + len(self.assumptions.annual_cash_flows))}11>=0, 0)-1', '0.0'),
            ('Average Annual Return', f'=AVERAGE(\'Cash Flows\'!C5:{get_column_letter(2 + len(self.assumptions.annual_cash_flows))}5)', '$#,##0'),
            ('Total Undiscounted Return', f'=SUM(\'Cash Flows\'!B7:{get_column_letter(2 + len(self.assumptions.annual_cash_flows))}7)', '$#,##0'),
        ]
        ...
        for col in range(1, 4):
            ...
            ws.cell(row=3, column=col).fill = PatternFill(start_color=COLORS['header'], 
                                                           end_color=COLORS['header'], 
                                                           fill_type='solid')
        ...
        for row, (metric, formula, format_str) in enumerate(metrics, 4):
            ...
            ws[f'B{row}'].fill = PatternFill(start_color=COLORS['output'], 
                                            end_color=COLORS['output'], 
                                            fill_type='solid')
        ...
        ws['B12'] = '=IF(AND(B4>0, B5>DiscountRate), "ACCEPT - Project creates value", "REJECT - Project destroys value")'
