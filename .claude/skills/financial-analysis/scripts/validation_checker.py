                    ...
                    if cell.value:
                        ...
                        if isinstance(cell.value, str):
                            ...
                            if cell.value.startswith("="):
                                ...
                                if suspicious_numbers and sheet_name not in [
                                    "Assumptions",
                                    "Cover",
                                ]:
                    ...
                    if cell.value and "revenue" in str(cell.value).lower():
                        ...
                        for offset in range(1, 5):
                            if cell.column + offset <= ws.max_column:
                                ...
                                if (
                                    isinstance(adjacent_cell.value, (int, float))
                                    and adjacent_cell.value < 0
                                ):
                                    negative_revenue.append(
                                        f"{sheet_name}!{adjacent_cell.coordinate}"
                                    )
    ...
    def generate_report(self) -> dict:
        ...
        summary = {
            ...
            "critical": sum(
                ...
                if r.level == ValidationLevel.CRITICAL and r.status == "FAIL"
            ),
            "errors": sum(
                1 for r in self.results if r.level == ValidationLevel.ERROR and r.status == "FAIL"
            ),
            "warnings": sum(
                1 for r in self.results if r.level == ValidationLevel.WARNING and r.status == "FAIL"
            ),
        }

        summary["pass_rate"] = (
            summary["passed"] / summary["total_checks"] if summary["total_checks"] > 0 else 0
        )
