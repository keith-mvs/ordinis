...
class BlackScholesCalculator:
    ...
    def all_greeks(self, params: OptionParameters) -> Dict[str, float]:
        ...
        result = {
            'price': self.call_price(S, K, T, r, sigma) if is_call 
                    else self.put_price(S, K, T, r, sigma),
            'delta': self.delta_call(S, K, T, r, sigma) if is_call 
                    else self.delta_put(S, K, T, r, sigma),
            'gamma': self.gamma(S, K, T, r, sigma),
            'vega': self.vega(S, K, T, r, sigma),
            'theta': self.theta_call(S, K, T, r, sigma) if is_call 
                    else self.theta_put(S, K, T, r, sigma),
            'rho': self.rho_call(S, K, T, r, sigma) if is_call 
                   else self.rho_put(S, K, T, r, sigma)
        }
