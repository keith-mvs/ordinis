...
class ModelValidator:
    ...
    def check_required_sheets(self) -> ValidationResult:
        ...
        required_sheets = {
            'Cover', 'Assumptions', 'Validation', 'Documentation'
        }
    ...
    def check_hardcoded_values(self, exclude_sheets: List[str] = None) -> ValidationResult:
        ...
        if exclude_sheets is None:
            exclude_sheets = ['Cover', 'Assumptions', 'Documentation', 
                            'Validation', 'Charts']
            ...
            for row in ws.iter_rows():
                for cell in row:
                    if cell.value and isinstance(cell.value, str) and cell.value.startswith('='):
                        ...
                        if ('VLOOKUP' in formula or 'XLOOKUP' in formula) and 'IFERROR' not in formula and 'IFNA' not in formula:
                            cell_ref = f'{sheet_name}!{cell.coordinate}'
                            formulas_without_error_handling.append(
                ...
                for cell in row:
                    if cell.value and isinstance(cell.value, str) and cell.value.startswith('='):
                        ...
                        if cell.coordinate in cell.value:
                            cell_ref = f'{sheet_name}!{cell.coordinate}'
                            potential_circular.append(
                                f'{cell_ref}: References itself'
                            )
                ...
                for cell in row:
                    if cell.column == 2 and cell.value is not None:  # Column B typically has values
                        if cell.fill.start_color.rgb != INPUT_COLOR:
                            issues.append(
                                f'Assumptions!{cell.coordinate}: Input cell not yellow'
                            )
                ...
                for cell in row:
                    if isinstance(cell.value, (int, float)):
                        ...
                        if cell.number_format == 'General':
                            cell_ref = f'{sheet_name}!{cell.coordinate}'
                            issues.append(
                                f'{cell_ref}: Number has no specific format'
                            )
    ...
    def check_balance_sheet_balance(self) -> ValidationResult:
        ...
        for row_idx, row in enumerate(ws.iter_rows(max_row=50), 1):
            ...
            if 'total assets' in cell_value:
                assets_row = row_idx
            elif 'total liabilities and equity' in cell_value or \
                 'total liabilities & equity' in cell_value:
        ...
        if not assets_row or not liab_equity_row:
            ...
            result = ValidationResult(
                ...
                check_description='Balance sheet balances',
                status='INFO',
                details=(f'Balance check rows identified: Assets row {assets_row}, '
    ...
    def generate_report(self, output_path: str = None) -> str:
        ...
        report_lines = [
            '='*80,
            f'FINANCIAL MODEL VALIDATION REPORT',
        ...
        for result in self.results:
            ...
            if result.affected_cells and len(result.affected_cells) <= 10:
                ...
                report_lines.append(f'Affected cells: {len(result.affected_cells)} total (showing first 10)')
