...
### 1. Repository Root Lockdown (Hard Boundary)

The repository root is **write-restricted**.

**Only the following files are permitted at root:**

* `README.md` — project-level overview only
* `pyproject.toml`
* `requirements.txt`
...
**Only the following directories are permitted at root:**
...
**Any other file or directory at root is a policy violation and must be rejected or relocated immediately.**
No exceptions. No temporary files. No task byproducts.

### 2. Task Output Placement (Mandatory Routing)

All task-generated artifacts **must be written outside the repository root** and routed explicitly by artifact class:

| Artifact Class                | Destination  | Notes                                       |
| ----------------------------- | ------------ | ------------------------------------------- |
| Permanent documentation       | `docs/`      | Architecture, specs, governance, guides     |
| Analytical deliverables       | `reports/`   | Backtests, evaluations, summaries           |
| Generated / transient outputs | `artifacts/` | Logs, dumps, scratch, exports (git-ignored) |
| Agent / session records       | `.claude/`   | Session logs, compacted transcripts         |

**Task execution MUST declare its output target before writing files.**
If no target is declared, default routing is **`artifacts/`**.
