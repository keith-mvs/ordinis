...
try:
    from indicator_calculator import IndicatorConfig, TechnicalIndicators
except ImportError:
    # If run as script, adjust import
    import os

    sys.path.insert(0, os.path.dirname(__file__))
    from indicator_calculator import TechnicalIndicators


def plot_comprehensive_analysis(
    data: pd.DataFrame, save_path: str = None, title: str = "Technical Analysis"
):
    """
    ...
    ax1.plot(data.index, bb["upper"], label="BB Upper", linestyle="--", color="gray", alpha=0.7)
    ax1.plot(data.index, bb["middle"], label="BB Middle", linestyle="--", color="blue", alpha=0.7)
    ax1.plot(data.index, bb["lower"], label="BB Lower", linestyle="--", color="gray", alpha=0.7)
    ax1.fill_between(data.index, bb["upper"], bb["lower"], alpha=0.1, color="gray")

    # Moving Averages
    ma = indicators["trend"]["ma"]
    ax1.plot(
        data.index, ma["SMA_fast"], label=f"SMA {ti.config.ma_fast}", color="orange", alpha=0.7
    )
    ax1.plot(data.index, ma["SMA_slow"], label=f"SMA {ti.config.ma_slow}", color="red", alpha=0.7)
    ...
    downtrend = ~uptrend
    ax1.scatter(data.index[uptrend], psar[uptrend], color="green", s=20, alpha=0.5, label="PSAR Up")
    ax1.scatter(
        data.index[downtrend], psar[downtrend], color="red", s=20, alpha=0.5, label="PSAR Down"
    )
    ...
    ax6.axhline(y=25, color="blue", linestyle="--", linewidth=1, alpha=0.7, label="Trend Threshold")
...
def main():
    ...
    plot_comprehensive_analysis(
        data, save_path=output_file, title=f"Technical Analysis - {input_file.stem}"
    )
