---
title: Synapse (Retrieval Engine) — RAG Architecture
date: 2025-12-18
version: 1.0
type: service architecture
description: >
  Retrieval-augmented generation (RAG) system architecture, indexing, and retrieval components (Synapse).
...
flowchart TD
  subgraph Indexing
    KB[Knowledge Base (Markdown)] --> KBX[KBIndexer]
    CODE[Codebase] --> CX[CodeIndexer]
    KBX --> CH[Chunking]
    CX --> CH
    CH --> EMB[Embedders]
  end

  EMB --> VDB[(ChromaDB Vector DB)]

  Q[Query] --> RET[Retrieval Engine]
  RET <--> VDB
  RET --> CTX[Context Snippets]

  CTX --> CORTEX[Cortex]
  CTX --> SE[SignalEngine (SignalCore)]
  CTX --> RE[RiskEngine (RiskGuard)]
  CTX --> AE[AnalyticsEngine (ProofBench)]

  SY[Synapse (Engine Wrapper)] -. uses .-> RET
  SY -. uses .-> EMB
  SY -. uses .-> VDB
  SY -. via .-> HX[Helix]
```
...
| **Engine Integration** | `src/ordinis/ai/synapse/` | Shared Synapse engine wrapper used by Cortex and tools |
...
| [SignalEngine (SignalCore)](signalcore.md) | Retrieval integration targets |
| [NVIDIA Integration](nvidia-integration.md) | Model setup for AI-enhanced engine workflows |
| [NVIDIA Blueprint Integration](../inbox/archive/nvidia-blueprint-integration.md) | Distillery for model optimization |
