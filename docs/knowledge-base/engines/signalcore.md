---
title: SignalEngine (SignalCore) — System Architecture
date: 2025-12-18
version: 1.0
type: engine architecture
description: >
  System-level architecture for SignalEngine (SignalCore) and its boundaries with OrchestrationEngine, StreamingBus, RiskEngine (RiskGuard), ExecutionEngine (FlowRoute), PortfolioEngine, AnalyticsEngine (ProofBench), and GovernanceEngine; includes supporting AI services (Cortex, Synapse, Helix).
source_of_truth: ../inbox/documents/system-specification.md
---

# SignalEngine (SignalCore) — Architecture

## Overview

The **SignalEngine (SignalCore)** generates trading signals from market data and engineered features. Signals are advisory outputs that must be gated by **RiskEngine (RiskGuard)** and executed by **ExecutionEngine (FlowRoute)** under **OrchestrationEngine** control.

For canonical engine roles and naming, treat `docs/knowledge-base/inbox/documents/system-specification.md` as the source of truth.
...
```mermaid
flowchart TD
  MD[Market Data Adapters] --> SB[StreamingBus]

  ORCH[OrchestrationEngine]
  GE[GovernanceEngine]

  SB --> SE[SignalEngine (SignalCore)]
  SE --> RE[RiskEngine (RiskGuard)]
  RE --> EE[ExecutionEngine (FlowRoute)]
  EE --> BRK[Broker/Exchange]
  BRK --> EE

  EE --> PE[PortfolioEngine]
  EE --> AE[AnalyticsEngine (ProofBench)]
  AE --> LE[LearningEngine]

  ORCH -. orchestrates .-> SE
  ORCH -. orchestrates .-> RE
  ORCH -. orchestrates .-> EE
  ORCH -. orchestrates .-> PE
  ORCH -. orchestrates .-> AE
  ORCH -. orchestrates .-> LE

  GE -. preflight/audit .-> ORCH
  GE -. preflight/audit .-> RE
  GE -. audit .-> EE

  ORCH -. advisory .-> CX[Cortex (LLM Reasoning)]
  ORCH -. retrieval .-> SY[Synapse (Retrieval Engine)]
  SY -. context .-> CX
  CX --> HX[Helix (LLM Provider Layer)]
  SY --> HX
```

**Legacy diagram (below)**: the ASCII block predates the System Specification terminology.
Mapping: `Cortex orchestration engine` -> `OrchestrationEngine` + `Cortex`, `SignalCore ML engine` ->
`SignalEngine (SignalCore)`, `RiskGuard rule engine` -> `RiskEngine (RiskGuard)`, `FlowRoute execution engine` ->
`ExecutionEngine (FlowRoute)`, `ProofBench validation engine` -> `AnalyticsEngine (ProofBench)`.
...
- **Does NOT bypass** RiskEngine (RiskGuard) rule checks
- Configures and reviews downstream engines only
- All proposals are advisory until validated by AnalyticsEngine (ProofBench)
...
The engine architecture is **logical**, not physical. Valid implementations include:
...
```
SignalEngine (SignalCore) integration with Claude Code:
...
This document focuses on the SignalEngine (SignalCore) subsystem and its most-critical adjacent components/services:

| Component | Canonical Term | Primary Responsibility |
|--------|-------|----------------------|
| **Cortex** | Cortex (LLM Reasoning Engine) | Intelligence, research, strategy design (advisory) |
| **SignalCore** | SignalEngine (SignalCore) | Quantitative signal generation |
| **RiskGuard** | RiskEngine (RiskGuard) | Deterministic risk constraints |
| **FlowRoute** | ExecutionEngine (FlowRoute) | Broker integration, order management |
| **ProofBench** | AnalyticsEngine (ProofBench) | Simulation, evaluation, reporting |
...
| Document | Notes |
|----------|-------|
| [System Specification](../inbox/documents/system-specification.md) | Canonical engine roles, interfaces, and terminology |
| [Phase 1 Production Architecture](system-architecture.md) | Persistence, safety, orchestration, observability |
| [AnalyticsEngine (ProofBench) Architecture](proofbench.md) | Simulation/backtesting architecture |
| [AnalyticsEngine (ProofBench) Guide](proofbench-guide.md) | Running backtests and interpreting results |
| [Synapse (Retrieval Engine)](synapse.md) | Retrieval (RAG) architecture and integration points |
| [Helix (LLM Provider Layer)](helix.md) | LLM provider routing/caching/fallback |
| [NVIDIA AI Integration](nvidia-integration.md) | Model integration across engines |
