...
```
Current Structure:
├── ordinis/analysis/technical/indicators/    # VolatilityIndicators, Oscillators
├── ordinis/engines/signalcore/features/      # TechnicalIndicators class
├── ordinis/quant/                            # NEW: gs_quant_adapter functions
└── ordinis/engines/riskguard/models/         # gsquant_risk.py (API-focused)
```

**Issue**: Technical indicators now exist in **3 separate locations** with different APIs:

1. Class-based (`TechnicalIndicators.rsi()`)
2. Standalone functions (`ordinis.quant.rsi()`)
3. Feature-rich classes (`Oscillators.rsi()` with `OscillatorSignal`)
...
```python
# ordinis/analysis/technical/core.py
from ordinis.quant import bollinger_bands, rsi, macd, volatility

# Deprecate TechnicalIndicators class methods in favor of ordinis.quant
```
...
```python
from ordinis.quant import bollinger_bands, rsi
```
...
```python
# Missing from ordinis/application/strategies/__init__.py:
...
```python
# ordinis/engines/signalcore/models/bollinger_rsi_confluence.py
class BollingerRSIConfluenceModel(Model):
    """Uses ordinis.quant for calculations"""
```
...
1. `src/ordinis/application/strategies/__init__.py` - Add export
2. `src/ordinis/engines/signalcore/features/technical.py` - Consider deprecation
3. `src/ordinis/analysis/technical/indicators/` - Consider consolidation
