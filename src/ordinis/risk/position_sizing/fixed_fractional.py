...
class FixedFractionalSizing:
    ...
    def percent_equity(
        self, account_value: float, entry_price: float, allocation_percent: float = 0.10
    ) -> PositionSizeResult:
        """
...
class FixedRatioSizing:
    ...
    def calculate_units(self, account_value: float) -> int:
        ...
        # Fixed ratio formula
        units = int(np.floor(np.sqrt(2 * account_value / self.delta + 0.25) - 0.5))
    ...
    def calculate_position(
        self, account_value: float, entry_price: float, contract_value: float = 1.0
    ) -> dict[str, float]:
        """
...
class AntiMartingale:
    ...
    def update(self, trade_result: float) -> float:
        ...
        if trade_result > 0:
            ...
            self.consecutive_losses = 0
            self.current_risk = min(self.current_risk + self.win_increase, self.max_risk)
        else:
            self.consecutive_losses += 1
            self.consecutive_wins = 0
            self.current_risk = max(self.current_risk - self.loss_decrease, self.min_risk)
...
class PositionSizingEngine:
    ...
    def calculate(
        ...
        if method == SizingMethod.PERCENT_RISK:
            ...
            return self.fixed_fractional.percent_risk(self.account_value, entry_price, stop_price)

        if method == SizingMethod.PERCENT_EQUITY:
            return self.fixed_fractional.percent_equity(self.account_value, entry_price)

        if method == SizingMethod.PERCENT_VOLATILITY:
            if atr is None:
                raise ValueError("ATR required for volatility method")
            return self.fixed_fractional.percent_volatility(self.account_value, entry_price, atr)
