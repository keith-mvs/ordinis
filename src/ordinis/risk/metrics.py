...
Based on KB: docs/knowledge-base/domains/risk/risk-metrics-library.md
"""
...
class RiskMetrics:
    ...
    Implements all standard risk and performance metrics including:
    - Return metrics (total, annualized, CAGR)
    - Volatility metrics (vol, downside deviation, upside deviation)
    - Drawdown metrics (max DD, duration, Ulcer Index)
    - Value at Risk (historical, parametric, CVaR)
    - Risk-adjusted returns (Sharpe, Sortino, Calmar, Omega, etc.)
    ...
    def volatility(self, returns: np.ndarray) -> float:
        ...
        return float(np.std(returns, ddof=1) * self.sqrt_periods)

    def downside_deviation(self, returns: np.ndarray, mar: float = 0.0) -> float:
        """
        ...
        if len(downside) == 0:
            return 0.0
        return float(np.std(downside, ddof=1) * self.sqrt_periods)
    ...
    def upside_deviation(self, returns: np.ndarray, mar: float = 0.0) -> float:
        ...
        if len(upside) == 0:
            return 0.0
        return float(np.std(upside, ddof=1) * self.sqrt_periods)
    ...
    def sharpe_ratio(self, returns: np.ndarray) -> float:
        ...
        if vol == 0:
            return 0.0
        return float(np.mean(excess) / vol * self.sqrt_periods)

    def sortino_ratio(self, returns: np.ndarray, mar: float = 0.0) -> float:
        """Sortino Ratio (excess return / downside deviation)."""
        excess = np.mean(returns) - mar / self.periods
        downside_dev = self.downside_deviation(returns, mar / self.periods)
        if downside_dev == 0:
            return 0.0
        return float(excess * self.periods / downside_dev)
    ...
    def information_ratio(self, returns: np.ndarray, benchmark: np.ndarray) -> float:
        """Information Ratio (excess return / tracking error)."""
        active = returns - benchmark
        tracking_error = np.std(active, ddof=1)
        if tracking_error == 0:
            return 0.0
        return float(np.mean(active) / tracking_error * self.sqrt_periods)

    def treynor_ratio(self, returns: np.ndarray, benchmark: np.ndarray) -> float:
    ...
    def alpha(self, returns: np.ndarray, benchmark: np.ndarray) -> float:
        """Jensen's Alpha (annualized)."""
        b = self.beta(returns, benchmark)
        r_p = self.annualized_return(returns)
        r_m = self.annualized_return(benchmark)
        return float(r_p - (self.rf + b * (r_m - self.rf)))

    def r_squared(self, returns: np.ndarray, benchmark: np.ndarray) -> float:
    ...
    def calculate_all(
        ...
        return PerformanceMetrics(
            ...
            avg_win=float(np.mean(trades[trades > 0])) if np.any(trades > 0) else 0,
            avg_loss=float(np.mean(trades[trades < 0])) if np.any(trades < 0) else 0,
        )
