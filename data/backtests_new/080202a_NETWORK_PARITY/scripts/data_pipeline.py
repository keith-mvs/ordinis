...
from config import MASSIVE_DATA_DIR, HISTORICAL_DATA_DIR, BacktestingConfig
...
class DataPipelineResult:
    ...
    def get_returns_matrix(self, timeframe: str = "1D", min_coverage: float = 0.8) -> pd.DataFrame:
        """Get returns matrix for all symbols.

        Args:
            timeframe: Target timeframe ("1D" or other)
            min_coverage: Minimum fraction of non-NaN values required per symbol (0.8 = 80%)

        Returns:
            DataFrame with aligned returns, sparse symbols dropped first
        """
        returns_dict = {}
        ...
        # Drop sparse symbols (e.g., BBBY that got delisted)
        if len(df) > 0:
            coverage = df.notna().mean()
            sparse_symbols = coverage[coverage < min_coverage].index.tolist()
            if sparse_symbols:
                logger.debug(f"Dropping sparse symbols: {sparse_symbols}")
                df = df.drop(columns=sparse_symbols)
...
class HistoricalDataPipeline:
    """
    Load data from historical period files (downloaded from Massive API).

    These files contain daily OHLCV data for multiple symbols across different
    market regime periods (2004, 2008, 2010, 2017, 2024, 2025).
    """

    def __init__(self, data_dir: Path = HISTORICAL_DATA_DIR):
        """Initialize pipeline with historical data directory."""
        self.data_dir = Path(data_dir)
    ...
    def get_available_periods(self) -> list[str]:
        ...
        periods = []
        for gz_file in sorted(self.data_dir.glob("*.csv.gz")):
            period_name = gz_file.stem.replace(".csv", "")
            periods.append(period_name)
        return periods

    def load_period(self, period_name: str) -> pd.DataFrame | None:
        """Load all data for a specific period."""
        if period_name in self._period_cache:
            return self._period_cache[period_name]

        gz_path = self.data_dir / f"{period_name}.csv.gz"
        if not gz_path.exists():
            logger.warning(f"Period file not found: {gz_path}")
            return None

        try:
            df = pd.read_csv(gz_path, compression="gzip")
            df["timestamp"] = pd.to_datetime(df["timestamp"])
            self._period_cache[period_name] = df
            return df
        except Exception as e:
            logger.error(f"Failed to load {gz_path}: {e}")
            return None

    def load_period_universe(
        self,
        period_name: str,
        symbols: list[str] | None = None,
    ) -> DataPipelineResult:
        """
        Load data for all symbols in a period.

        Args:
            period_name: Name of the period (e.g., "2024_recent")
            symbols: Optional filter for specific symbols

        Returns:
            DataPipelineResult with loaded data
        """
        df = self.load_period(period_name)
        if df is None:
            return DataPipelineResult(data={})

        result = DataPipelineResult(data={})
        available_symbols = df["ticker"].unique().tolist()
        ...
        for symbol in target_symbols:
            ...
            market_data = MarketData(
                ...
                bars=sym_df,
                timeframe="1D",
                start_date=sym_df.index.min().to_pydatetime() if len(sym_df) > 0 else None,
                end_date=sym_df.index.max().to_pydatetime() if len(sym_df) > 0 else None,
            )

            result.data[symbol] = market_data
            result.symbols_loaded.append(symbol)
            result.total_bars += len(sym_df)

        if result.symbols_loaded:
            all_starts = [d.start_date for d in result.data.values() if d.start_date]
            all_ends = [d.end_date for d in result.data.values() if d.end_date]
            result.date_range = (min(all_starts) if all_starts else None, max(all_ends) if all_ends else None)

        logger.info(f"Loaded period {period_name}: {len(result.symbols_loaded)} symbols, {result.total_bars} bars")
        return result

    def load_all_periods(
        self,
        symbols: list[str] | None = None,
    ) -> dict[str, DataPipelineResult]:
        """Load data from all available periods."""
        results = {}
        for period_name in self.get_available_periods():
            results[period_name] = self.load_period_universe(period_name, symbols)
        return results
